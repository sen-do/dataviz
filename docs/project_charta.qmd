...
The visualization maps three value dimensions:

- **Cognitive:** betweenness centrality encoded as node size lets users immediately identify structural bridge actors without reading labels — the most analytically important information is pre-attentively visible.
- **Communicative:** community membership encoded as colour separates domain clusters (legal, financial, personal) at a glance; edge width encodes co-occurrence strength so dense connections stand out; labels are shown only for top nodes to avoid clutter.
- **Experiential:** a fullscreen force-directed canvas with hover tooltips, click-to-detail sidebar, and direct links to DOJ source documents supports both open-ended exploration and targeted provenance checking.
